...
nonspam = "Easy mate, * guess the quick drink was bit ambitious."
...
def test_smoke(text=message,model=svc,threshold=threshold):
...
def test_format(text=message,model=svc,threshold=threshold):
    label,propensity = score.score(text,model,threshold)
...
def test_pred(text=message,model=svc,threshold=threshold):
    label,propensity=score.score(text,model,threshold)
    assert label == False or label == True

# check if the label value is in {0,1}
def test_prop(text=message,model=svc,threshold=threshold):
    label,propensity=score.score(text,model,threshold)
    assert propensity>=0 and propensity<=1

# if threshold is 0, prediction becomes 1
def test_threshold_0(text=message,model=svc,threshold=0):
    label,propensity=score.score(text,model,threshold)
    assert label == 1

# if threshold is 1, prediction becomes 0
def test_threshold_1(text=message,model=svc,threshold=1):
    label,propensity=score.score(text,model,threshold)
    assert label == 0
...
# testing obvious ham
def test_nonspam(text=nonspam,model=svc,threshold=threshold):
